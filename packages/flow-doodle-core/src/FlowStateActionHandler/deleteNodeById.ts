import { FlowConnection, FlowState, FlowStateDataScopes, FlowStateType, FlowStateView, FlowViewListEntryNode } from '@flow-doodle/core/FlowState/FlowTypes'
    ...
    if(didDelete) {
        ...
        if(connectionRename) {
            ...
            if(toRenameConnections.length > 0) {
                fs = fs.update('connections', connections => {
                    const cons: FS['connections'] = [...(connections as FS['connections']) || []]
